...
GPU: NVIDIA GeForce RTX 3060 Laptop (6 GB) · CUDA driver 12.7 · PyTorch cu126 wheels.
...
| 6 — Walk-Forward Pipeline (frozen VAE) | – | – | – |
| 7 — Pair Persistence & Turnover | – | – | – |
| 8 — Transaction Costs | – | – | – |
| 9 — Benchmark (VAE vs GARCH vs Correlation vs B&H) | – | – | – |
| 10 — Results Compilation & Write-Up | – | – | – |

---

## Current phase

**Phases 1–5 complete.** The full VAE → HDBSCAN → cointegration → market-neutral strategy chain runs
end-to-end on the corrected frozen-VAE foundation, with a clean out-of-sample (2020+) baseline backtest.

**Next: Phase 6 — Walk-Forward Pipeline (frozen VAE).** Run the whole downstream chain on a rolling
quarterly schedule across 2020+ using the *frozen* 2015–2017 VAE: each rebalance re-encodes the latest
windows → re-clusters (HDBSCAN) → re-cointegrates (Engle–Granger) → re-filters → trades the next quarter
with the Phase 5 strategy, chained into one continuous out-of-sample equity curve. The VAE weights never
update. Not started — begin here when ready.
